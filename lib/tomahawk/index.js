...
function Tomahawk (resolverNames, app) {
    ...
    function loadResolverData (name) {
        var resolversPath = __dirname + '/resolvers/' + name + '/content/',
    ...
    function loadResolverModule (data) {
        var resolver = require(data.main);
    ...
    (function () {
        ...
        for (i = 0, len = resolverNames.length; i < len; i++) {
            ...
            resolvers.push(loadResolverModule(resolverData));
        }
    }());
}
