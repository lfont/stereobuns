...
var YoutubeResolver = Tomahawk.extend(new TomahawkResolver(), {
	settings:
	{
		name: 'YouTube',
		...
		weight: 90,
		timeout: 15,
        audioscrobblerApiKey: 'f9e5c8edc3025b33d88589d9f1ecb142'
	},
	...
	parseURLS: function(rawUrls)
	{
		...
		if (matches) {
			...
			var urlArray = urls.split(new RegExp("," + matches[0], "i"));
	...
	resolve: function(qid, artist, album, title)
	{
		...
		Tomahawk.asyncRequest(apiQuery, function(resp) {
            ...
            function lookup (i, qid, result) {
                var responseItem = resp.data.items[i];
                Tomahawk.asyncRequest(responseItem.player['default'], function (html) {
                    var finalResult;
                    result.url = that.parseVideoUrlFromYtPage(html);
                    if (result.url && result.url !== undefined) {
                        ...
                        if (stop === 0) {
                            ...
                            var resolveReturn = {
                                results: [finalResult],
                                weight: YoutubeResolver.settings.weight,
                                qid: qid
                            };
			...
			if (resp.data.totalItems !== 0) {
				...
				for (var i = 0; i < Math.min(limit, resp.data.totalItems); i++) {
	...
	cleanupAndParseTrack: function(title, searchString)
	{
		...
		if (!result.parsed) {
			if (title.toLowerCase().indexOf(searchString.toLowerCase()) !== -1) {
				result.parsed = this.parseCleanTrack(title.replace(new RegExp(searchString, "gi"), searchString.concat(" :")));
			}
			else
			{
				...
				if (title.toLowerCase().indexOf(tryMatch.toLowerCase()) !== -1) {
					...
					result.parsed = this.parseCleanTrack(title.replace(new RegExp(tryMatch, "gi"), replaceWith));
				}
			}
		}
	...
	handleItemResponse: function(qid, searchString, data)
	{
		...
		var artistLookupUrl = "http://ws.audioscrobbler.com/2.0/?method=artist.search&api_key=" + this.settings.audioscrobblerApiKey + "&format=json&limit=1&artist=";
		var that = this;
		var count = 0;
		for (var i = 0; i < Math.min(data.totalItems, 50); i++) {
			...
			if (this.getTrack(track, searchString, true)) {
                ...
                result.artworkUrl = data.items[i].thumbnail['sqDefault'];
				if (that.qualityPreference === 0) {
					result.linkUrl += "&hd=1";
				}
				results.push(result);
        ...
        function artistPageLookup (url, userdata) {
            Tomahawk.asyncRequest(url, function (html) {
                var url = that.parseVideoUrlFromYtPage(html);
                if (url) {
            ...
            Tomahawk.asyncRequest(url, function (response) {
                if (response.results !== undefined && response.results.artistmatches.artist !== undefined) {
                    ...
                    for (var i = 0; i < ids.length; i++) {
                        artistPageLookup(results[ids[i]].url, {
                            id: ids[i],
                            artist: artist,
                            results: results
                        });
		...
		for (var artistKey in artists) {
            if (artists.hasOwnProperty(artistKey)) {
                artistLookup(artistLookupUrl+artistKey, artists[artistKey]);
            }
		}
	},
