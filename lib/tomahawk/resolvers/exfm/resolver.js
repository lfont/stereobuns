...
module.exports = {
  ...
  search: function (qid, searchString) {
    var _this = this,
        ...
        url = 'http://ex.fm/api/v3/song/search/' + searchString + '?' +
      ...
      if (response.results > 0) {
        response.songs.forEach(function (song) {
          ...
          if (dTitle.toLowerCase().indexOf(searchString.toLowerCase() !== -1)) {
            result.track = ((dTitle !== '') ? dTitle : searchString).trim();
            result.artist = dArtist;
            result.trackId = metadata.name + song.id;
            result.source = metadata.name;
            result.url = song.url;
            result.artworkUrl = song.image.small;
            result.linkUrl = 'http://ex.fm/song/' + song.id;
            results.push(result);
          }
        });
      }
