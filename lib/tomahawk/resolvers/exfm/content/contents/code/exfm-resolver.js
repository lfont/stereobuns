...
var ExfmResolver = Tomahawk.extend(new TomahawkResolver(), {
    settings: {
        name: 'Ex.fm',
        icon: 'exfm-icon.png',
        weight: 100,
    ...
    resolve: function (qid, artist, album, title) {
        ...
        // Build search query for ex.fm
            url = "http://ex.fm/api/v3/song/search/" + encodeURIComponent(title) + "?start=0&results=20&client_id=" + this.settings.exfmClientId;
        ...
        Tomahawk.asyncRequest(url, function (response) {
            ...
            Tomahawk.addTrackResults({
                qid: qid,
                weight: ExfmResolver.settings.weight,
                results: results
            });
        });
    },
