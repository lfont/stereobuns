...
module.exports = {
  ...
  search: function (qid, searchString) {
    ...
    var apiQuery = 'http://api.soundcloud.com/tracks.json?' +
    ...
    Tomahawk.asyncRequest(apiQuery, function (resp) {
      ...
      for (i = 0; i < resp.length; i++) {
        ...
        if (track.indexOf(" - ") !== -1 && track.slice(track.indexOf(" - ") + 3).trim() !== "") {
          ...
          result.track = resp[i].title.trim();
          result.artist = resp[i].user.username;
        } else {
          continue;
        }

        result.trackId = metadata.name + resp[i].id;
        result.source = metadata.name;
        result.duration = resp[i].duration / 1000;
        result.url = resp[i].stream_url + '.json?client_id=' + _this.settings.apiKey;
